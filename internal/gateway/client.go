...
// RequestDetail 单次请求的最近详情（与网关 metrics.RequestDetail 对应）。
...
// RequestDetails 拉取指定模型的最近请求详情。
func (c *Client) RequestDetails(ctx context.Context, model string, limit int) ([]RequestDetail, error) {
	if limit <= 0 {
		limit = 100
	}
	path := "/v1/stats/details?limit=" + strconv.Itoa(limit)
	if model != "" {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("网关 /v1/stats/details 返回 HTTP %d: %s", resp.StatusCode, truncate(string(raw), 200))
	}
	var out struct {
		Enabled bool            `json:"enabled"`
	...
	if err := json.Unmarshal(raw, &out); err != nil {
		return nil, fmt.Errorf("解析请求详情失败: %w", err)
	}
